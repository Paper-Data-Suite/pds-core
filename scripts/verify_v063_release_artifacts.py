"''Inspect exact pds-core v0.6.3 wheel and source-distribution artifacts."""
...
REQUIRED_STARTER_RESOURCES: Final[frozenset[str]] = frozenset(
    {
        "pds_core/starter_data/standards/ap_csp_fall_2023_library.json",
        "pds_core/starter_data/standards/njsls_clks_2020_library.json",
        "pds_core/starter_data/standards/njsls_csdt_2020_library.json",
...
def _assert_exact_starter_resources(names: set[str], source: str) -> None:
    actual = {
        name
        for name in names
        if name.startswith("pds_core/starter_data/standards/")
        and name.endswith(".json")
    }
    if actual != REQUIRED_STARTER_RESOURCES:
        missing = sorted(REQUIRED_STARTER_RESOURCES - actual)
        unexpected = sorted(actual - REQUIRED_STARTER_RESOURCES)
        raise ValueError(
            f"{source}: starter standards resources do not match the exact "
            f"v0.6.3 release surface; missing={missing!r}, "
            f"unexpected={unexpected!r}."
        )


def verify_wheel(path: Path) -> ReleaseArtifactIdentity:
    """Verify exact v0.6.3 wheel identity and required packaged surfaces."""
    ...
    try:
        with zipfile.ZipFile(candidate) as archive:
            ...
            try:
                ...
                raise ValueError(
                    "Wheel is missing a readable pds_core/__init__.py."
                ) from error
            ...
            if missing_modules:
                raise ValueError(
                    f"Wheel is missing required Core modules: {missing_modules}"
                )
            _assert_exact_starter_resources(names, "wheel")
...
def verify_sdist(path: Path) -> ReleaseArtifactIdentity:
    """Verify exact v0.6.3 source-distribution identity and required source files."""
    candidate = path.resolve()
    ...
    try:
        with tarfile.open(candidate, mode="r:gz") as archive:
            ...
            if archive_root != f"pds_core-{VERSION}":
                raise ValueError(f"Unexpected sdist archive root: {archive_root!r}.")
            ...
            if starter_resources != REQUIRED_STARTER_RESOURCES:
                missing = sorted(REQUIRED_STARTER_RESOURCES - starter_resources)
                unexpected = sorted(starter_resources - REQUIRED_STARTER_RESOURCES)
                raise ValueError(
                    "sdist: starter standards resources do not match the exact "
                    f"v0.6.3 release surface; missing={missing!r}, "
                    f"unexpected={unexpected!r}."
                )
...
def main() -> int:
    ...
    print("pds-core v0.6.3 release artifacts: PASS")
    return 0
