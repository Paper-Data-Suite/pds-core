...
The audit found four related **Partial parity** rows. Therefore PDS Core does
not currently claim complete CLI/menu parity. GitHub issue
[#151](https://github.com/Paper-Data-Suite/pds-core/issues/151) defines the
focused implementation work for atomic compound standard creation and batch,
membership-only profile mutations.
...
| Standards -> Add standard with one or more lettered subparts | `add_standard` collects and validates parent plus every subpart in memory, then `_write_library` once | Repeated `standards add` -> `handle_standards_add` | Menu makes one atomic write preserving parent/subpart order. CLI writes once per record; a later invalid/duplicate/write failure leaves an accepted prefix. No group rollback. | Each call returns `0` or `1`; no group result exists. | Partial parity | [#151](https://github.com/Paper-Data-Suite/pds-core/issues/151): `standards add-batch <path>`. |
...
| Profiles -> Edit -> Add standards to profile (several selected) | Same method builds the complete appended tuple and writes once | Repeated singular `profile add-standard` calls | CLI can preserve order and metadata, but each accepted member is committed separately; later failure leaves partial membership. | Per-call `0`/`1`; no batch result or rollback. | Partial parity | [#151](https://github.com/Paper-Data-Suite/pds-core/issues/151): `profile add-standards`. |
...
| Profiles -> Edit -> Remove standards from profile (several selected) | Same method computes the complete reduced tuple and writes once | Repeated singular `profile remove-standard` calls | CLI commits each removal independently; later missing/invalid/write failure leaves partial removal. | Per-call `0`/`1`; no batch result or rollback. | Partial parity | [#151](https://github.com/Paper-Data-Suite/pds-core/issues/151): `profile remove-standards`. |
| Profiles -> Edit -> Replace profile standards | `replace_profile_standards` uses `dataclasses.replace(profile, standards=...)`, then `replace_standards_profile` and one write | `standards profile replace <profile_id> [metadata] [--standard ...]` -> `handle_profile_replace` | Menu changes only membership and preserves all metadata. CLI performs full-record replacement and clears every omitted metadata field. Re-supplying metadata can reach the state, but lacks membership-only safety. | `0` success; `1` validation/missing/write error. | Partial parity | [#151](https://github.com/Paper-Data-Suite/pds-core/issues/151): `profile set-standards`. |
