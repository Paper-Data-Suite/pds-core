"""CLI coverage for the 2020 NJSLS-CS&DT starter pack."""
...
def test_csdt_starter_list_preview_and_validate_are_discoverable(
    tmp_path: Path,
    ...
    assert "2020 NJSLS-CS&DT" in out
    assert "grade bands: K-2, 3-5, 6-8, 9-12" in out
    assert "163 standards" in out
    ...
    assert "Design Thinking" in out
    assert "computer_science_9_12_2020_njsls_csdt" in out
    assert "design_thinking_9_12_2020_njsls_csdt" in out
    ...
    assert "(163 standards, 2 profiles, frameworks: 1)" in out
    assert err == ""
    assert list(tmp_path.iterdir()) == []


def test_csdt_starter_install_reports_expected_counts(
    tmp_path: Path,
    capsys: pytest.CaptureFixture[str],
) -> None:
    code, out, err = run_cli(
    ...
    assert "Standards: 163 added, 0 skipped, 0 overwritten." in out
    ...
    assert library.frameworks[0].framework_id == "njsls_csdt_2020"


def test_cli_validate_all_reports_both_bundled_packs(
    tmp_path: Path,
    capsys: pytest.CaptureFixture[str],
) -> None:
    code, out, err = run_cli(
