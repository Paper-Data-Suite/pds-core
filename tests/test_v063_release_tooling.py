...
from scripts.verify_v063_release_artifacts import (
    REQUIRED_MODULES,
...
from scripts.write_v063_release_checksums import write_checksums
...
def _write_wheel(
    ...
    *,
    ...
    omit_resource: str | None = None,
) -> Path:
    path = root / WHEEL_NAME
    dist_info = "pds_core-0.6.3.dist-info"
    with zipfile.ZipFile(path, "w") as archive:
        ...
        archive.writestr("pds_core/__init__.py", b'__version__ = "0.6.3"\n')
        for resource in sorted(REQUIRED_STARTER_RESOURCES):
            if resource != omit_resource:
                archive.writestr(resource, b"{}\n")
...
def test_v063_release_artifact_verifier_accepts_exact_artifacts(
    tmp_path: Path,
) -> None:
    ...
    assert wheel_identity.version == "0.6.3"
    assert sdist_identity.version == "0.6.3"


def test_v063_release_artifact_verifier_rejects_runtime_dependency(
    tmp_path: Path,
) -> None:
...
def test_v063_release_artifact_verifier_rejects_missing_required_module(
    tmp_path: Path,
) -> None:
    missing = "pds_core/starter_standards.py"
...
def test_v063_release_artifact_verifier_rejects_missing_starter_resource(
    tmp_path: Path,
) -> None:
    missing = "pds_core/starter_data/standards/ap_csp_fall_2023_library.json"
    wheel = _write_wheel(tmp_path, omit_resource=missing)
    with pytest.raises(ValueError, match="starter standards resources"):
        verify_wheel(wheel)

    sdist = _write_sdist(tmp_path, omit_resource=missing)
    with pytest.raises(ValueError, match="starter standards resources"):
        verify_sdist(sdist)


def test_v063_checksum_writer_is_deterministic_and_exact(tmp_path: Path) -> None:
...
def test_v063_checksum_writer_rejects_wrong_artifact_name(tmp_path: Path) -> None:
